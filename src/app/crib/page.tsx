...
export default function CribPage() {
  const [cribData, setCribData] = useState<CribData | null>(null);
  const [loading, setLoading] = useState(true);
  const [selectedDummy, setSelectedDummy] = useState<string>('');
  const [selectedCribType, setSelectedCribType] = useState<string>('');
  ...
  const loadData = async () => {
    try {
      const response = await fetch('/data/crib-data.json');
      const data = await response.json();
      setCribData(data);
      setLoading(false);
    } catch (error) {
      console.error('Failed to load crib data:', error);
      setLoading(false);
    }
  };

  const getSelectedDummy = () => {
    if (!selectedDummy || !cribData) return null;
    return cribData.test_dummies.find(d => d.name === selectedDummy);
  };

  const getSelectedCribType = () => {
    if (!selectedCribType || !cribData) return null;
    return cribData.crib_types.find(t => t.type === selectedCribType);
  };

  const filteredDummies = cribData?.test_dummies.filter(d =>
    d.age.toLowerCase().includes(searchAge.toLowerCase())
  ) || [];

  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' }}>
      <div className="container mx-auto p-4 space-y-6">
        ...
        <Card className="bg-white/95 backdrop-blur">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-2xl" style={{ color: '#4facfe' }}>
                  婴儿床设计工具 V1.0.0
                </CardTitle>
                <CardDescription>
                  GB 28007 & EN 1130 & ASTM F1169 标准婴儿床设计参考
                </CardDescription>
              </div>
        ...
        <Tabs defaultValue="standards" className="space-y-4">
          <Card className="bg-white/95 backdrop-blur">
            <CardHeader>
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="types">床型分类</TabsTrigger>
          ...
          <TabsContent value="standards">
            <Card className="bg-white/95 backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>婴儿床安全标准详细信息</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {cribData && (
                  <div className="space-y-4">
                    <div>
                      <Label>标准名称</Label>
                      <p className="text-lg font-semibold">{cribData.standard_info.name}</p>
                    </div>
                    <div>
                      <Label>描述</Label>
                      <p>{cribData.standard_info.description}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label>版本</Label>
                        <p className="font-medium">{cribData.standard_info.version}</p>
                      </div>
                      <div>
                        <Label>生效日期</Label>
                        <p className="font-medium">{cribData.standard_info.effective_date}</p>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <TabsContent value="dummies">
            <Card className="bg-white/95 backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>用于婴儿床测试的标准假人规格</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select value={selectedDummy} onValueChange={setSelectedDummy}>
                      ...
                      <SelectContent>
                        {cribData?.test_dummies.map((dummy) => (
          ...
          <TabsContent value="types">
            <Card className="bg-white/95 backdrop-blur">
              <CardHeader>
                <CardTitle>婴儿床类型</CardTitle>
                <CardDescription>不同类型婴儿床的适用范围和规格</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="type-select">选择床型</Label>
                    <Select value={selectedCribType} onValueChange={setSelectedCribType}>
                      <SelectTrigger id="type-select">
                        <SelectValue placeholder="选择婴儿床类型" />
                      </SelectTrigger>
                      <SelectContent>
                        {cribData?.crib_types.map((type) => (
                ...
                {selectedCribType && getSelectedCribType() && (
                  <Card className="bg-blue-50 border-2 border-blue-200">
                    <CardHeader>
                      <CardTitle className="text-lg">{getSelectedCribType()!.name_cn}</CardTitle>
                      <CardDescription>{getSelectedCribType()!.name_en}</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <p className="font-semibold">
                            {getSelectedCribType()!.min_weight} - {getSelectedCribType()!.max_weight} kg
                          </p>
                        </div>
                        <div>
                          <Label>适用年龄</Label>
                          <p className="font-semibold">
                            {getSelectedCribType()!.min_age} - {getSelectedCribType()!.max_age}
                          </p>
                        </div>
                      </div>
                      <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {getSelectedCribType()!.features.map((feature) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {cribData?.crib_types.map((type) => (
                    <Card
                      key={type.type}
                      className="cursor-pointer hover:shadow-md transition-shadow"
                      onClick={() => setSelectedCribType(type.type)}
                    >
          ...
          <TabsContent value="safety">
            <Card className="bg-white/95 backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>婴儿床安全测试要求和安全特性</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold mb-3">安全测试要求</h3>
                  <div className="space-y-2">
                    {cribData?.safety_requirements.map((req) => (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {cribData?.safety_features.map((feature, idx) => (
                      <Card
                        key={idx}
